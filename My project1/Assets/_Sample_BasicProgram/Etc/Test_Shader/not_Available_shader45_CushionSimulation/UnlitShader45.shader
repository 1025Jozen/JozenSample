Shader "Custom/UnlitShader45"
{
    ...
    float4 frag(v2f_customrendertexture i) : SV_Target
    {
        ...
        float2 self = tex2D(_SelfTexture2D, uv);// 現在の位置のテクセルをフェッチ
        return float4(self.r * 0.99, 0, 0, 0);  //徐々に凹みが元に戻る
    }
    ...
    float4 frag_interactive(v2f_customrendertexture i) : SV_Target
    {
        float2 uv = i.globalTexcoord;
        float2 self = tex2D(_SelfTexture2D, uv);// 現在の位置のテクセルをフェッチ   
        return float4(clamp((self.r - 0.01) * 1.0001,_InteractiveDisplacement,0), 0, 0, 0); //徐々に凹む
    }  
    ENDCG
