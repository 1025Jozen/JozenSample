...
namespace Shader_Sample
{
    ...
    public class CushionController : MonoBehaviour
    {
        ...
        private void FixedUpdate() {//Updateはダメ　ライフサイクル要参照  
            _customRenderTexture.ClearUpdateZones();//UpdateZoneはリセット
            _customRenderTexture.Update(_iterationPerFrame);//更新したいフレーム数を指定して更新
        }
        ...
        private void OnTriggerStay(Collider other) {
            ...
            var hitLocalPos = transform.InverseTransformPoint(hitPos);//ワールド座標からローカル座標に変換  
            if (LocalPointToUV(hitLocalPos, out var uv)) {//hitLocalPosをuvに変換して代入　成功したら実行
